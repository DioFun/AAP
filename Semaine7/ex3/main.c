...
int main(int argc, char *argv[])
{
  ...
  srand((unsigned int)time(NULL));
	...
	Test_Fn("QUICK SORT LIBC", quickSort_libc, tab, MAX_ELT/10, m[MODE_TAB_ORDONNE] );
	Test_Fn("QUICK SORT LIBC", quickSort_libc, tab, MAX_ELT/10, m[MODE_TAB_ALEATOIRE] );
	Test_Fn("QUICK SORT LIBC", quickSort_libc, tab, MAX_ELT/10, m[MODE_TAB_INVERSE] );

	Test_FnV2("QUICK SORT LIBC", quickSort_libc, tab, 512, m[MODE_TAB_ORDONNE] );
	Test_FnV2("QUICK SORT LIBC", quickSort_libc, tab, 512, m[MODE_TAB_ALEATOIRE] );
	Test_FnV2("QUICK SORT LIBC", quickSort_libc, tab, 512, m[MODE_TAB_INVERSE] );
