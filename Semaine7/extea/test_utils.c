...
T_data genData(T_elt e, T_list * pE) {
	T_data d;
	d.elt = e; 
	d.pElt = pE; 
	return d; 
 ...
 *                  </ul>
 */
void Initialiser (T_list* l, int n, int mode)
{
    int i;
    *l=NULL;
    ...
    for (i = 0; i < n; i++){
        switch (mode)
        {
        case MODE_LIST_ORDONNE :  // Ordonné
           *l = addNode(genData(n-i, NULL), *l);
            break;
        case MODE_LIST_ALEATOIRE :  //  Aléatoire
            *l = addNode(genData(rand(), NULL), *l); // NB : il y aura potentiellement des chiffres identiques dans le tableau
            break;
        case MODE_LIST_INVERSE :  // Inverse
            *l = addNode(genData(i, NULL), *l);
            break;
        }
        // showList(*l); printf("\n");    
    }
 ...
 */
int estOrdonne (T_list l, int n, int strict)
{
    int i;
    if (strict)
        for (i = 1; i < n && l->pNext->data.elt > l->data.elt; i++) {
            l = l->pNext;
        }
    else {
        for (i = 1; i < n && l->pNext->data.elt >= l->data.elt; i++){
 ...
 *
 */
void Test_Fn ( const char * nom,  T_pFToTest fn, T_list* l, int nmax, T_mode m)
{
    int i;
    clock_t start, stop;
    unsigned long nbH;
    ...
    // for (i=0; i<nmax; i++) {
		switch (m.mode) {
			case MODE_LIST_ORDONNE : Initialiser(l, i, MODE_LIST_ORDONNE);break; 
			case MODE_LIST_ALEATOIRE : Initialiser(l, i, MODE_LIST_ALEATOIRE);break; 
			case MODE_LIST_INVERSE : Initialiser(l, i, MODE_LIST_INVERSE);break; 
        ...
        start = clock();
        // showList(*l);
		d = genData(m.x, l); // les données : le tableau Table, l'élément proposé dans le mode choisi 
		(*fn)(d, i); // appel de la fonction à tester  

        stop = clock();
		if ( (m.checkOrder) && (!estOrdonne(*l, i, m.mode == MODE_LIST_ORDONNE || m.mode == MODE_LIST_INVERSE))) {
 ...
 *
 */
void Test_FnV2 ( const char * nom,  T_pFToTest fn, T_list * l, int nmax, T_mode m)
{
    int i;
    unsigned long nbH;
	T_data d; 
